...
export const PATCH: RequestHandler = async (event) => {
	...
	try {
		const team = getTeamStore(rt.db).update(id, body.name);
		if (!team) return apiError(404, 'team not found');
		audit(rt, event, 'team.update', `id=${id}`);
...
export const DELETE: RequestHandler = (event) => {
	...
	const rt = getRuntime();
	const id = event.params.id;
	if (!getTeamStore(rt.db).remove(id)) return apiError(404, 'team not found');
	audit(rt, event, 'team.delete', `id=${id}`);
