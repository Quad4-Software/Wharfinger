...
export const GET: RequestHandler = (event) => {
	...
	const rt = getRuntime();
	const projects = rt.telemetry.projects().map((p) => ({
		...p,
		publicKey: undefined,
		key: p.publicKey,
...
export const POST: RequestHandler = async (event) => {
	...
	if (rt.telemetry.projects().length >= 100) return apiError(422, 'project limit reached');
	const p = rt.telemetry.createProject(name, asString(body.platform, 40));
	audit(rt, event, 'telemetry.project.create', name);
	return apiJson({
		project: {
			...p,
			publicKey: undefined,
