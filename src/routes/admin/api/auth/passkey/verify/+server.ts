...
export const POST: RequestHandler = async (event) => {
	...
	const fail = async (username: string | null = null) => {
		await rt.protection.record(ip, username, false);
		await rt.audit.log({ username, action: 'auth.login.fail', detail: 'passkey', ip });
	...
	const pending = await rt.passkeys.peekChallenge(challenge, 'login');
	if (!pending) return fail();
	const cred = await rt.passkeys.byCredentialId(response.id);
	if (!cred) return fail();
	...
	const user = await rt.users.byId(cred.userId);
	if (!user) return fail();
	...
	const delay = await rt.protection.usernameDelayMs(user.username, policy);
	...
	if (user.totpEnabled) {
		...
		if (!ok && row) ok = await rt.users.consumeBackupCode(row.id, hashToken(code));
	...
	const outcome = await rt.passkeys.recordUse(cred.id, info.newCounter, info.credentialBackedUp);
	if (outcome === 'cloned') {
		await rt.audit.log({
			userId: user.id,
			username: user.username,
		...
		await rt.protection.record(ip, user.username, false);
		return apiError(401, GENERIC);
	}
	if (outcome === 'missing') return fail(user.username);
	if (!(await rt.passkeys.consumeChallenge(challenge, 'login'))) return fail(user.username);

	await rt.protection.record(ip, user.username, true);
