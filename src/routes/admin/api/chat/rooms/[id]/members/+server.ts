...
export const GET: RequestHandler = async (event) => {
	...
	try {
		await rt.chat.roomInfoFor(event.params.id, user.id);
		return apiJson({ members: await rt.chat.memberIds(event.params.id) });
	} catch (err) {
		return chatFail(err);
	}
};
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		await rt.chat.addMember(event.params.id, user.id, target);
		await audit(rt, event, 'chat.member.add', `room=${event.params.id} user=${target}`);
		return apiJson({ ok: true, room: await rt.chat.roomInfoFor(event.params.id, user.id) });
	} catch (err) {
		return chatFail(err);
	}
};
