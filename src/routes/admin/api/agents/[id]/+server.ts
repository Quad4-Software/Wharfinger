...
export const PATCH: RequestHandler = async (event) => {
	...
	await rt.db.prepare('UPDATE agents SET name = ? WHERE id = ?').run(name, agent.id);
	await audit(rt, event, 'agents.rename', `id=${agent.id} name=${name}`);
...
export const DELETE: RequestHandler = async (event) => {
	...
	const agent = await rt.agents.get(event.params.id);
	if (!agent) return apiError(404, 'agent not found');
	await rt.agents.revoke(agent.id);
	await audit(rt, event, 'agents.revoke', `id=${agent.id} name=${agent.name}`);
	return apiJson({ ok: true });
};
