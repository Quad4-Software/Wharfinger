...
export const DELETE: RequestHandler = (event) => {
	...
	const match = rt.invites.recent(500).find((i) => i.tokenHash.startsWith(prefix));
	if (!match) return apiError(404, 'unknown invite');
	rt.invites.revoke(match.tokenHash);
	audit(rt, event, 'invites.revoke', `kind=${match.kind} role=${match.role}`);
