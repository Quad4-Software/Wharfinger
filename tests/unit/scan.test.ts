...
function mkApp(deploys: DeployStore, name = 'app', source: AppSource = IMAGE): DeployApp {
	return deploys.createApp({ name, agentId: 'agent-1', source }).app;
}
...
describe('ScanStore reports', () => {
	it('creates, lists, and completes a report with a summary rollup', () => {
		const { scans } = stores();
		const rep = scans.createReport({ appId: 'a1', target: 'img:1' });
		expect(rep.status).toBe('queued');
		expect(scans.activeForApp('a1')?.id).toBe(rep.id);

		scans.markRunning(rep.id, rep.startedAt);
		expect(scans.report(rep.id)?.status).toBe('running');

		const done = scans.complete(rep.id, 'done', {
		...
		expect(scans.findings(rep.id)).toHaveLength(4);
		expect(scans.activeForApp('a1')).toBeNull();
		expect(scans.latestForApp('a1')?.id).toBe(rep.id);
		expect(scans.listForApp('a1')).toHaveLength(1);
	});

	it('caps findings at the per-report bound', () => {
		const { scans } = stores();
		const rep = scans.createReport({ appId: 'a1', target: 'img:1' });
		const many = Array.from({ length: MAX_FINDINGS_PER_REPORT + 100 }, (_, i) =>
			finding('low', `pkg${i}`, `CVE-${i}`)
		);
		scans.complete(rep.id, 'done', { findings: many });
		expect(scans.findings(rep.id)).toHaveLength(MAX_FINDINGS_PER_REPORT);
	});

	it('ignores a second completion for the same report', () => {
		const { scans } = stores();
		const rep = scans.createReport({ appId: 'a1', target: 'img:1' });
		scans.complete(rep.id, 'done', { findings: [finding('critical')] });
		const again = scans.complete(rep.id, 'failed', { error: 'late failure' });
		expect(again?.status).toBe('done');
		expect(again?.summary.critical).toBe(1);
	});

	it('returns latestPerApp one row per app', () => {
		const { scans } = stores();
		scans.createReport({ appId: 'a1', target: 'i1', now: 1000 });
		const newer = scans.createReport({ appId: 'a1', target: 'i2', now: 2000 });
		scans.createReport({ appId: 'a2', target: 'i3', now: 1500 });
		const rows = scans.latestPerApp();
...
describe('ScanStore recommendations', () => {
	...
	it('inserts, dedupes, and keeps dismissed recs suppressed', () => {
		const { scans } = stores();
		let open = scans.sync('a1', [rec('add-healthcheck'), rec('run-non-root')]);
		expect(open).toHaveLength(2);

		// Same evaluation twice: still two rows, no duplicates.
		open = scans.sync('a1', [rec('add-healthcheck'), rec('run-non-root')]);
		expect(open).toHaveLength(2);

		const hc = open.find((r) => r.kind === 'add-healthcheck')!;
		expect(scans.setStatus(hc.id, 'dismissed')).toBe(true);

		// A dismissed rec never reappears, even when re-evaluated.
		open = scans.sync('a1', [rec('add-healthcheck'), rec('run-non-root')]);
		expect(open.map((r) => r.kind)).toEqual(['run-non-root']);
		expect(scans.rec(hc.id)?.status).toBe('dismissed');
	});

	it('deletes resolved open recs and reopens applied recs on regression', () => {
		const { scans } = stores();
		scans.sync('a1', [rec('add-healthcheck'), rec('pin-image-tag')]);
		// Condition for pin-image-tag cleared: its open row is removed.
		let open = scans.sync('a1', [rec('add-healthcheck')]);
		expect(open.map((r) => r.kind)).toEqual(['add-healthcheck']);

		const hc = open[0];
		expect(scans.setStatus(hc.id, 'applied')).toBe(true);
		expect(scans.rec(hc.id)?.appliedAt).toBeTruthy();
		// Regression: the same evaluation result reopens the applied rec.
		open = scans.sync('a1', [rec('add-healthcheck')]);
	...
	it('scopes dedupe per app and reports open counts', () => {
		const { scans } = stores();
		scans.sync('a1', [rec('run-non-root')]);
		scans.sync('a2', [rec('run-non-root'), rec('add-healthcheck')]);
		const counts = scans.openCounts();
		expect(counts.get('a1')).toBe(1);
		expect(counts.get('a2')).toBe(2);
		expect(scans.allOpen()).toHaveLength(3);
		expect(scans.recsForApp('a2')).toHaveLength(2);
	});

	it('setStatus rejects a second transition', () => {
		const { scans } = stores();
		const [r] = scans.sync('a1', [rec('run-non-root')]);
		expect(scans.setStatus(r.id, 'applied')).toBe(true);
		expect(scans.setStatus(r.id, 'dismissed')).toBe(false);
	});
});

describe('evaluate', () => {
	const { deploys } = stores();
	const imageApp = mkApp(deploys, 'img-app', IMAGE);
	const pinnedApp = mkApp(deploys, 'pinned-app', {
		kind: 'image',
		url: 'registry.example.com/app:1.2.3'
	});
	const gitApp = mkApp(deploys, 'git-app', GIT);
	...
	it('flags an unpinned image tag and marks it fixable with a digest', () => {
	...
	it('flags a missing healthcheck as fixable when a port is known', () => {
		const withPorts = {
			...spec,
		...
		const healthy = deploys.updateApp(imageApp.id, {
	...
	it('flags published ports that bypass the routed domains', () => {
		const withPorts = {
			...spec,
			run: { ...spec.run, ports: [{ host: 8080, container: 3000 }] }
		};
	...
	it('flags the container running without a user directive', () => {
	...
	it('flags critical and high findings with top offenders in the detail', () => {
		const findings = [
...
describe('fixFor', () => {
	const { deploys } = stores();
	const app = mkApp(deploys, 'fix-app', IMAGE);
	const base = {
		id: 'rec_x',
		appId: app.id,
		dedupeKey: '',
	...
	it('builds a source patch that pins the digest', () => {
	...
	it('builds a healthcheck patch from the suggested port', () => {
	...
	it('refuses malformed data and non-fixable kinds', () => {
...
describe('enqueueScan and settleScanJob', () => {
	it('enqueues a scan job and dedupes while one is in flight', () => {
		const { deploys, jobs, scans } = stores();
		const app = mkApp(deploys, 'scan-app', IMAGE);
		const { job, report, deduped } = enqueueScan(deploys, jobs, scans, app.id, {});
		...
		const again = enqueueScan(deploys, jobs, scans, app.id, {});
		expect(again.deduped).toBe(true);
		expect(again.report.id).toBe(report.id);
	});

	it('rejects scans for git apps that were never deployed', () => {
		const { deploys, jobs, scans } = stores();
		const app = mkApp(deploys, 'never-dep', GIT);
		expect(() => enqueueScan(deploys, jobs, scans, app.id, {})).toThrow(/nothing to scan/);
	});

	it('settles a succeeded job into a done report and fresh recs', () => {
		const { deploys, jobs, scans, rt } = stores();
		const app = mkApp(deploys, 'settle-app', IMAGE);
		const { job, report } = enqueueScan(deploys, jobs, scans, app.id, {});

		const claimed = jobs.claim('scan', 'agent-1')!;
		expect(claimed.id).toBe(job!.id);
		expect(jobs.start(job!.id, claimed.lease)).toBe(true);
		markScanJobRunning(rt, job!.id);
		expect(scans.report(report.id)?.status).toBe('running');

		const digest = `registry.example.com/app@sha256:${'c'.repeat(64)}`;
		expect(
			jobs.succeed(job!.id, claimed.lease, {
				scanId: report.id,
				findings: [
		...
		settleScanJob(rt, job!.id);

		const done = scans.report(report.id)!;
		expect(done.status).toBe('done');
		expect(done.summary).toEqual({ critical: 1, high: 0, medium: 0, low: 1, unknown: 0 });
		expect(scans.findings(report.id)).toHaveLength(2);

		const recs = scans.recsForApp(app.id);
	...
	it('settles a failed job into a failed report', () => {
		const { deploys, jobs, scans, rt } = stores();
		const app = mkApp(deploys, 'fail-app', IMAGE);
		const { job, report } = enqueueScan(deploys, jobs, scans, app.id, {});
		const claimed = jobs.claim('scan', 'agent-1')!;
		jobs.start(job!.id, claimed.lease);
		jobs.fail(job!.id, claimed.lease, { error: 'trivy not installed' });
		// fail() requeues while attempts remain; force the terminal
		// transition to exercise settle.
		const retry = jobs.claim('scan', 'agent-1')!;
		jobs.start(job!.id, retry.lease);
		expect(jobs.fail(job!.id, retry.lease, { error: 'trivy not installed' }).status).toBe('failed');
		settleScanJob(rt, job!.id);
		const done = scans.report(report.id)!;
	...
	it('rejects a result whose scan id does not match the spec', () => {
		const { deploys, jobs, scans, rt } = stores();
		const app = mkApp(deploys, 'mismatch-app', IMAGE);
		const { job, report } = enqueueScan(deploys, jobs, scans, app.id, {});
		const claimed = jobs.claim('scan', 'agent-1')!;
		jobs.start(job!.id, claimed.lease);
		jobs.succeed(job!.id, claimed.lease, { scanId: 'scan_other', findings: [] });
		settleScanJob(rt, job!.id);
		expect(scans.report(report.id)?.status).toBe('failed');
	});
});
